...
import { ADDRESSES, SUBGRAPHS, DEX_CHAIN, getRpcUrl, EXPLORER_URL } from "@/packages/dex-config";
...
export { SUBGRAPHS, DEX_CHAIN, getRpcUrl, EXPLORER_URL };
...
/** True when the V2 AMM is deployed - gates the classic (constant-product) LP UI. */
export const v2Configured = (): boolean =>
  isSet(DEX.v2Factory) && isSet(DEX.v2Router);

/** True when a given subgraph endpoint is set - gates analytics/enrichment. */
export const subgraphConfigured = (url?: string): boolean =>
  Boolean(url && (url.startsWith("http") || url.startsWith("/api/")));
